...
Future.traverse(pageUrls) { url =>
  // Alejandro's suggestion: I can use a for loop instead of nested map()
  for {
    page <- getPage(url)
    val parsedImages = parseImages(page).toList
    _ <- Future.traverse(parsedImages) { imageUrl => downloadFileFut(fileUrl=imageUrl, outdir=os.pwd / cleanUrl(url)) }
  } yield ()
}
...
def downloadFileFut(fileUrl: String, outdir: os.Path): Future[Unit] = Future {
